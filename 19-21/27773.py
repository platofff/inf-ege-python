...
def iter(k1, k2, n):
    n -= 1
    if n > 0:
        return SimpleNamespace(sum=k1 + k2, l=[iter(k1 - 1, k2, n), iter(k1, k2 - 1, n), iter(k1 // 2, k2, n), iter(k1, k2 // 2, n)])
    return SimpleNamespace(sum=k1 + k2, l=[(k1 - 1, k2), (k1, k2 - 1), (k1 // 2, k2), (k1, k2 // 2)])

for s in reversed(range(11, 100)):
    combos = iter(10, s, 4)
    first_win_combos = []
        ...
        if petya1 not in first_win_combos:
            for vanya1 in petya1.l: # Ваня 1
                for petya2 in vanya1.l: # Петя 2
                    if petya2.sum <= 20: break # Петя не должен выиграть 2 ходом
                    for vanya2 in petya2.l: # Ваня 2
                        if sum(vanya2) <= 20: # Ваня выигрывает 2 ходом
                            w += 1
                            break
                    else: break
                else: break
